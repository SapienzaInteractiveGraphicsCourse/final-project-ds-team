...
var scene_character_jumping = false;
...
var idx_to_key = ["high wall", "hurdle"];
...
function init_environment()
{
	...
	scene_side_walls = [final_mesh_bricks, final_mesh_inner_cube];
}
...
function addCharacter()
{
	...
	console.log(scene_character_parts)
...
function addRow()
{
	...
	{
		case 3:
			obj1 = models["hurdle"].clone(); //if three obstacles are present it guarantees that at least one is a hurdle
			obj1.position.x += side_walls_positions[0];
			obj1.position.z -= displacement;
			scene.add(obj1);
		case 2:
			var element_type = Math.floor(Math.random() * 2);
...
function initCollisionModule()
{
	...
	raycasters.push(new THREE.Raycaster(scene_character_parts["Bone_belly"].position.clone(), direction, 0, 0.5));
	raycasters.push(new THREE.Raycaster(scene_character_parts["Bone_left_upper_leg"].position.clone(), direction, 0, 0.5));
	raycasters.push(new THREE.Raycaster(scene_character_parts["Bone_left_lower_leg"].position.clone(), direction, 0, 0.5));
	raycasters.push(new THREE.Raycaster(scene_character_parts["Bone_right_upper_leg"].position.clone(), direction, 0, 0.5));
	raycasters.push(new THREE.Raycaster(scene_character_parts["Bone_right_lower_leg"].position.clone(), direction, 0, 0.5));
	raycasters.push(new THREE.Raycaster(scene_character_parts["Bone_left_foot"].position.clone(), direction, 0, 0.5));
	raycasters.push(new THREE.Raycaster(scene_character_parts["Bone_right_foot"].position.clone(), direction, 0, 0.5));
}
...
function move_character(e)
{
	...
	else if(!scene_character_jumping && e.key=="ArrowUp")
	{
		...
		resetRunAnimationBuffer[scene_character_parts_keys[0]].onComplete(function ()
		{
		...
		tweenUp.to({y: scene_character.position.y+3}, 450);
